...
class deSpeckNet(nn.Module):
    def __init__(self, channels=1):
        super(deSpeckNet, self).__init__()
        features = 64
        num_of_layers = 17

        layers_1 = [nn.Conv2d(channels, features, kernel_size=3, stride=1, padding=1, bias=False),
                    nn.ReLU(True)]
        for _ in range(num_of_layers - 2):
            layers_1.append(nn.Conv2d(features, features, kernel_size=3, padding=1, bias=False))
            layers_1.append(nn.BatchNorm2d(features))
            layers_1.append(nn.ReLU(True))
        layers_1.append(nn.Conv2d(features, channels, kernel_size=3, stride=1, padding=1, bias=False))
        self.fcn_clean = nn.Sequential(*layers_1)

        layers_2 = [nn.Conv2d(channels, features, kernel_size=3, stride=1, padding=1, bias=False),
                    nn.ReLU(True)]
        for _ in range(num_of_layers - 2):
            layers_2.append(nn.Conv2d(features, features, kernel_size=3, padding=1, bias=False))
            layers_2.append(nn.BatchNorm2d(features))
            layers_2.append(nn.ReLU(True))
        layers_2.append(nn.Conv2d(features, channels, kernel_size=3, stride=1, padding=1, bias=False))
        self.fcn_noise = nn.Sequential(*layers_2)

    def forward(self, x):
        x = x.unsqueeze(dim=1)
        clean = self.fcn_clean(x)
        noise = self.fcn_noise(x)
        return clean[:, 0, :, :], noise[:, 0, :, :]
    ...
    for i in range(Ep):
        with tqdm(total=len(train_data)) as t:
            ...
            for img, label in train_data:
                ...
                clean, noise = model(img)
                restore = clean * noise

                afa = 1
                part1 = torch.mean(torch.square(clean - label)) * afa

                beta = 0.01
                part2 = torch.mean(torch.square(restore - img)) * beta
...
if __name__ == '__main__':
    ...
    Model_saved = './Weight/'
    reset = True
    MODEL = deSpeckNet().to("cuda" if torch.cuda.is_available() else "cpu")
    if not reset:
    ...
    scheduler = lrs.StepLR(optimize, step_size=10, gamma=(1 - 0.002))
    epoch = 50  # 30
