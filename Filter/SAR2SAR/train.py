...
class Data(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = skimage.util.noise.random_noise(label, mode='speckle', var=var)
    ...
    for i in range(Ep):
        with tqdm(total=len(train_data)) as t:
            ...
            for img, label in train_data:
                ...
                loss = torch.mean(clean - label + torch.exp(label - clean))
...
if __name__ == '__main__':
    ...
    Model_saved = './Weight/'
    reset = True
    MODEL = model.S2S_L().to("cuda" if torch.cuda.is_available() else "cpu")
    if not reset:
    ...
    optimize = optim.Adam(MODEL.parameters(), lr=1e-3, betas=(0.9, 0.99), eps=1e-08)
    MODEL, optimize = amp.initialize(MODEL, optimize, opt_level='O1')
    scheduler = lrs.MultiStepLR(optimize, milestones=[5, 10, 15], gamma=0.1)  # [5, 10] [0.1, 0.01]
    epoch = 50  # 30
