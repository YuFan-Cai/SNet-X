...
def valid(Model, Mode):
    ...
    input = img.ReadAsArray(0, 0, width, height)[0:256, 0:256]

    img = gdal.Open(Refer_img)
    width = img.RasterXSize
    height = img.RasterYSize
    label = img.ReadAsArray(0, 0, width, height)[0:256, 0:256]

    Model.eval()
    with torch.no_grad():
        input = torch.tensor(input).to("cpu").float().unsqueeze(dim=0)
    ...
    for i in range(Ep):
        with tqdm(total=len(train_data)) as t:
            ...
            for img, label in train_data:
                ...
                clean_A = Model(img, key=False)
                regress = torch.mean(torch.square(clean_A - label))
                loss = regress
                ...
                A_loss += regress.item()
                # B_loss += res.item()
                t.set_postfix(loss=show_loss / step, regression=A_loss / step, Res=B_loss / step)
                step += 1
                t.update(1)
            t.close()
